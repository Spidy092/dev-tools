...
router.post('/rename', upload.array('files'), enforceTotalSize, validateUploadedFiles, async (req, res) => {
  ...
  const renamer = (relativePath, index) => {
    ...
    const patterned = renamePattern
      ? renamePattern.replace(/{name}/g, parsed.name).replace(/{index}/g, index + 1)
      : parsed.name;
    ...
    if (organizeByExtension === 'true') {
      ...
      finalPath = flatInput
        ? path.posix.join(folder, newName + ext)
        : path.posix.join(folder, finalPath);
    }
  ...
  if (files.length === 1) {
    try {
      ...
      return res.send(await fs.promises.readFile(files[0].path));
    } catch (err) {
  ...
  try {
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      archive.append(await fs.promises.readFile(files[i].path), { name: uniquePath });
    }
    archive.finalize();
  } catch (err) {
